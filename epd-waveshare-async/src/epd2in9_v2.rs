...
impl RefreshMode {
    ...
    pub fn display_update_control_2(&self) -> &[u8] {
        match self {
            RefreshMode::Partial => &[0x0F],
            // Potentially 0xF7 for a 2-color update, and 0xC7 for a 4-color update.
            _ => &[0xC7],
        }
    }
}
...
impl<HW> Epd<HW> for Epd2In9V2<HW>
where
    HW: EpdHw,
{
    ...
    async fn init(&mut self, spi: &mut HW::Spi, mode: RefreshMode) -> Result<(), HW::Error> {
        ...
        self.send(spi, Command::DisplayUpdateControl1, &[0x00, 0x80]).await?;
    ...
    async fn set_refresh_mode(
        ...
        self.send(spi, Command::SetLutMagic, mode.lut_magic()).await?;
        self.send(spi, Command::SetGateDrivingVoltage, mode.gate_voltage()).await?;
        self.send(spi, Command::SetSourceDrivingVoltage, mode.source_voltage()).await?;
    ...
    async fn update_display(&mut self, spi: &mut HW::Spi) -> Result<(), <HW as EpdHw>::Error> {
        ...
        if let Some(mode) = self.refresh_mode {
            self.send(spi, Command::DisplayUpdateControl2, mode.display_update_control_2())
                .await?;
        } else {
    ...
    async fn write_image(
        ...
        self.send(spi, Command::WriteLowRam, image).await
    }
}
