...
pub struct PortraitBinaryBuffer<const S: usize> {
    size: Size,
    bytes_per_row: usize,
...
pub const fn portrait_binary_buffer_length(size: Size) -> usize {
    (size.width as usize / 8) * size.height as usize
}

impl <const S: usize> PortraitBinaryBuffer<S> {
    /// Creates a new [BinaryBuffer] with all pixels set to `BinaryColor::Off`.
    pub fn new(size: Size) -> Self {
        debug_assert_eq!(size.width % 8, 0, "Width must be a multiple of 8 for binary packing.");
        Self {
...
impl <const S: usize> Dimensions for PortraitBinaryBuffer<S> {
    fn bounding_box(&self) -> Rectangle {
        Rectangle::new(Point::zero(), self.size)
    }
}

impl <const S: usize> DrawTarget for PortraitBinaryBuffer<S> {
...
mod tests {
    ...
    #[test]
    fn test_portrait_binary_buffer_draw_iter_singles() {
        const SIZE: Size = Size::new(16, 4);
        const BUFFER_LENGTH: usize = portrait_binary_buffer_length(SIZE);
        let mut buffer = PortraitBinaryBuffer::<{BUFFER_LENGTH}>::new(SIZE);

        // Draw a pixel at the beginning.
        buffer.draw_iter([Pixel(Point::new(0,0), BinaryColor::On)]).unwrap();
        assert_eq!(buffer.data[0], 0b1);

        // Draw a pixel in the center.
        buffer.draw_iter([Pixel(Point::new(10, 2), BinaryColor::On)]).unwrap();
        assert_eq!(buffer.data[5], 0b100);

        // Draw a pixel at the end.
        buffer.draw_iter([Pixel(Point::new(15, 3), BinaryColor::On)]).unwrap();
        assert_eq!(buffer.data[7], 0b10000000);
    }

    #[test]
    fn test_portrait_binary_buffer_draw_iter_multiple() {
        const SIZE: Size = Size::new(16, 4);
        const BUFFER_LENGTH: usize = portrait_binary_buffer_length(SIZE);
        let mut buffer = PortraitBinaryBuffer::<{BUFFER_LENGTH}>::new(SIZE);
        ...
        buffer.draw_iter([
            ...
            Pixel(Point::new(1, 1), BinaryColor::On)]).unwrap();
    ...
    #[test]
    fn test_portrait_binary_buffer_draw_iter_out_of_bounds() {
        const SIZE: Size = Size::new(16, 4);
        const BUFFER_LENGTH: usize = portrait_binary_buffer_length(SIZE);
        let mut buffer = PortraitBinaryBuffer::<{BUFFER_LENGTH}>::new(SIZE);
        let previous_data = buffer.data.clone();
        ...
        buffer.draw_iter([
            ...
            Pixel(Point::new(0, 4), BinaryColor::On)]).unwrap();

        assert_eq!(buffer.data, previous_data, "Data should not change when drawing out-of-bounds pixels.");
    }
}